...
export default class Cable {
  ...
  constructor(channelName, options) {
    ...
    const channelName = (_.isString(channelName))
      ? channelName
      : (_.isString(options))
      ? options
      : 'root';
   ...
   */
  get channel() {
    return (_.isUndefined(this._.channel))
      ? ''
      : this._.channel;
  }
  ...
  get parentPath() {
    ...
    if (_.isUndefined(this._.parentPath)) {
      this._.parentPath = this.parents().join('-');
    }

    return this._.parentPath;
  }
  ...
  get uniquePath() {
    return (this.parentPath.length > 0)
      ? this.parentPath + '-' + this.channel + '-' + this.id
  ...
  parents() {
    ...
    while(cableParent) {
      parentIds.push(cableParent.channel);
  ...
  _channel(cable) {
    ...
    if (_.isUndefined(this[cable.channel])) {
      this._.channels.push(cable.channel);
    }

    this[cable.channel] = cable;
    return this;
  }
  ...
  _publish(...params) {
    ...
    if (__GRAPH__[this.uniquePath]) {
      return;
    }

    __GRAPH__[this.uniquePath] = true;
  ...
  broadcast(...params) {
    ...
    _[this._.options.invocation](() => {
      // TODO this works because we wait but still call all of the subscribes in one thread
      __GRAPH__ = {};
